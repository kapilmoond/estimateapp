      ...
      {selectedDesign && (
        <div className="mt-6 p-6 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="flex gap-4 mt-6">
            <button
              onClick={() => handleGenerateHTML(selectedDesign)}
              disabled={isGeneratingHTML}
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400"
            >
              {isGeneratingHTML ? 'Generating HTML...' : 'Generate HTML Version'}
            </button>
            
            {selectedDesign.htmlContent && (
              <button
                onClick={() => handleDownloadHTML(selectedDesign)}
            ...
            <button
              onClick={() => {
                navigator.clipboard.writeText(selectedDesign.designContent);
                alert('Design content copied to clipboard!');
              }}
